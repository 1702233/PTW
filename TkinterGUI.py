...
librarylabel = ttk.Label(game_frame, text="Library", style="TLabel", font=("TkDefaultFont", 33))
librarylabel.grid(row=1, rowspan=1, column=0, columnspan=2)

#make buttons for each game
for i in range(len(gamelist)):
    new_button = Button(game_frame, text=gamelist[i].rstrip("\n"),
                    command=lambda j=i: update_game(gamelist[j].rstrip("\n")))
    #new_label = Label(root, text="Call of Duty 4")
    #new_label.grid(row=1, column=4, columnspan=1)
    new_button.grid(row=i+3, column=1, columnspan=1, padx=2, pady=5)
